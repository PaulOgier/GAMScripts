...
python3 offboard_user.py --doit --user leaver@yourdomain.com \
  ...
  --backup-dir /Users/you/offboarding_backups
```

A mailbox backup can run to hundreds of gigabytes. Point this at a **local
path, never inside iCloud, Dropbox or Google Drive** — a synced folder will try
to upload the whole thing. If you need to move an existing backup, a `mv` on
the same volume is instant.
...
python3 offboard_user.py --doit --force --user leaver@yourdomain.com \
  --email-to successor@yourdomain.com \
  --reuse-email-backup /Users/you/offboarding_backups/mailboxes/leaver@yourdomain.com_20260721
```
...
- **Awkward filenames are safe on macOS.** A full Drive → disk → Drive round
  trip returned every one of these unchanged: `Notes: Draft v2? | final`,
  `Q3/Q4 Budget Review`, `Café ☕ 中文 Bespreking`, `Trailing space `, a
  200-character name, `.hidden budget`, `CON`, and a name with an embedded
  newline. rclone stores the awkward characters as full-width lookalikes on
  disk and converts them back on upload, so the names on disk look odd and the
  names in Drive are correct.
